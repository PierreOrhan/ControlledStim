...
full_protocol.generate(3,data_path + full_protocol.name)
...
loss_full = np.squeeze(t_full[0])
loss_sparse = np.squeeze(t_sparse[0])
loss_high_sparse = np.squeeze(t_high_sparse[0])
...
# plt.show()
plt.plot(loss_full)
plt.title("Loss for walk_length = %dwalk_length, isi = %isi in %scommunity" % (walk_length, isi, full_protocol.name))
plt.show()
plt.plot(loss_sparse)
plt.title("Loss for walk_length = %dwalk_length, isi = %isi in %scommunity" % (walk_length, isi, sparse_protocol.name))
plt.show()
plt.plot(loss_high_sparse)
plt.title("Loss for walk_length = %dwalk_length, isi = %isi in %scommunity" % (walk_length, isi, high_sparse_protocol.name))
plt.show()
