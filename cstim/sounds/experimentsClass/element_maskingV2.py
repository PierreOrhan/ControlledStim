## Supports for the V2 version of ANN dataset
# Exactly similar to element_masking just changing the storage format of the latent_time_reduction
# and mask_indices variable, in order to obtain much faster and compact datasets.
...
def mask_and_latentV2(sequence_data_set_dir: str):
    ...
    for seq in range(sequences.shape[0]):
        ...
        # V2 format:
        import zarr as zr
        zg = zr.open_group(sequence_data_set_dir/ sequence["wav_path"].replace(".wav",".idxmasks"), mode="w")
        zg.array("elements",["element"],chunks=(None,))
        ltrs = np.stack([np.where(latent_time_reduction_blocks[i,:])[0][[0,-1]] for i in range(latent_time_reduction_blocks.shape[0])])
        zg.array("element_idxs",ltrs,chunks=(None,None))
        zg.array("sampled_negative_indices",sampled_negative_indices, chunks=(None,None,None))
        mti = np.stack([np.where(mask_time_indices[i,:])[0][[0,-1]] for i in range(latent_time_reduction_blocks.shape[0])])
        zg.array("mask_time_idx",mti,chunks=(None,None))

        ## We update the masks info_path in the dataframe
        sequences.loc[seq,"mask_info_path"] = str(sequence["wav_path"].replace(".wav",".idxmasks"))
    # update the dataset info csv:
    sequences.to_csv(sequence_data_set_dir / "trials.csv")



def mask_and_latent_BalancedNegativesV2(sequence_data_set_dir: str):
    """Preprocessing for self-supervised learning. Masking elements for contrastive learning using wav2vec2.
        Negatives strategy:
...
def mask_latentV2(sequence_data_set_dir: str):
