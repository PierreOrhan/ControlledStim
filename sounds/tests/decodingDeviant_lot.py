...
for idp,(low_freq,high_freq) in enumerate(zip(tones_fs[:1],tones_fs[-1:])):
    for k in lot_patterns.keys():
        ...
        for deviant in range(4):
            rs += [RandRegRand_LOT_deviant(name=k+"_lot_Deviant_a"+"low"+"_b"+"high"+"_deviant-"+str(deviant)+"_id-"+str(idp),
                                       lot_seq=k,
                                       tones_fs=tones_fs,
                                       deviant=deviant,
        ...
        rs += [RandRegRand_LOT_orig(name=k + "_lot_Orig_a" + "low" + "_b" + "high"+"_id-"+str(idp),
        ...
        for deviant in range(4):
            rs += [RandRegRand_LOT_deviant(name=k+"_lot_Deviant_a"+"high"+"_b"+"low"+"_deviant-"+str(deviant)+"_id-"+str(idp),
                                       lot_seq=k,
                                       tones_fs=tones_fs,
                                       deviant=deviant,
                                       motif_repeat=motif_repeat,
        ...
        rs += [RandRegRand_LOT_orig(name=k+"_lot_Orig_a"+"high"+"_b"+"low"+"_id-"+str(idp), #isi-"+str(isi)+"_seqisi-"+str(seq_isi)+"_deviant-"+str(0)
                                   lot_seq=k,
                                   tones_fs=tones_fs,
                                   motif_repeat=motif_repeat,
                                   isi=isi,sequence_isi=seq_isi)]
