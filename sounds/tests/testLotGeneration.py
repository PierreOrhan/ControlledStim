...
for idfreq_high,freq_high in enumerate(tones_fs[::2]):
    for idfreq_low, freq_low in enumerate(tones_fs[::2]):
        ...
        seq_isi = 0
        isi = 0
        rs += [RandRegRand_LOT_deviant_BoundFixedPool(name="RandReg_lot_freqHigh"+str(idfreq_high)+"_freqLow"+str(idfreq_low), #isi-"+str(isi)+"_seqisi-"+str(seq_isi)+"_deviant-"+str(0)
                                   lot_seq="pairs",
                                   tones_fs=tones_fs,
                                   deviant=0,
                                   motif_repeat=3,
