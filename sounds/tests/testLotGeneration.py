...
for seq_isi in np.arange(0,0.2,step=0.05):
    for isi in np.arange(0,0.3,step=0.05):
        rs += [RandRegRand_LOT(name="RandReg_lot_isi-"+str(isi)+"_seqisi-"+str(seq_isi),lot_seq="pairsAndAlt2",tones_fs=np.logspace(np.log(50),np.log(400),num=50),
                           motif_repeat=10,isi=isi,sequence_isi=seq_isi)]
lp = ListProtocol_independentTrial(rs)
lp.generate(n_trial=5,output_dir=output_dir)
